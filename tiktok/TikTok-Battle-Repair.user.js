...
function findGroups(
    ...
    function createPanel() {
        if (hidden || !document.body || document.getElementById(PANEL_ID)) return;
    ...
    setInterval(() => { if (!document.hidden) refreshPanelStatus(); }, 1000);
    setInterval(observeMissingLayoutSEI, 4200);
    setInterval(observeOpponentData, 4700);
    setInterval(observeTwoVTwo, 5200);
    setInterval(observeGroupCohost, 5700);
    setInterval(automaticTwoVTwoTick, 3000);
    setInterval(automaticGroupCohostTick, 3200);
    // Pre-battle automatic mutation disabled after live regression.
    setInterval(automaticTick, 3000);
    setInterval(automaticCohostNameTick, 3200);
